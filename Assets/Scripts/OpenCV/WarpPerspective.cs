...
public class WarpPerspective : MonoBehaviour
{
    ...
    public void PreCalculateTransformMatrix()
    {
        ...
        var percentWidth = 0.32;  //tham số dùng để bóp đầu lại
        var percentHeight = -0.15;  // tham số dùng để ngắn lại chiê
        var scaleAfterW = 0.21f;
        var ratH = 0.2;
        //var height = 

        var scale = (w - 2 * w * percentWidth + 2 * w * scaleAfterW) / (w - 2 * w * percentWidth);
        var newW = w * scale;
        var newH = (h - h * percentHeight);
        var difH = newH * scale - newH;
        ...
        dst_corner.put(0, 0,
                0 + w * percentWidth - scaleAfterW * w, 0-cutH,
                w - w * percentWidth + scaleAfterW * w, 0-cutH,
                w + (newW - w) / 2, (h - h * percentHeight + difH / 2),
                0 - (newW - w) / 2, (h - h * percentHeight + difH / 2));
