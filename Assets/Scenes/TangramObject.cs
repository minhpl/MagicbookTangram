...
{
    public class TangramObject
    {
        int xPos, yPos;
        string type;
        public Scalar HSVmin, HSVmax, lower_HSVMin, lower_HSVMax;
        Scalar Color;
        public Scalar ColorRGB;
        public TangramObject()
        {
        ...
        public TangramObject(string name)
        {
            setType (name);
        ...
        public void setColor (Scalar colorHSV)
        { 
            Color = colorHSV;

            //Imgproc.cvtColor(hsv, rgb, Imgproc.COLOR_HSV2RGB);
            //var cRGB = rgb.get(0, 0);
            //ColorRGB = new Scalar(colorRGB[0], colorRGB[1], colorRGB[2]);            
        } 
    }
}